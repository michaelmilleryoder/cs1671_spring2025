...
def text_to_csv(filepath):
    ''' 
    Please read in a text file at the `filepath` argument (in our case, `spanish_test.txt`) and produce a CSV file with the following columns:
    ...
    * character_length: the number of characters (length) of the line. See the Python string functions or pandas string functions.

    Save this CSV file as `<filepath>.csv`, in our case, `spanish_test.csv`
    '''
